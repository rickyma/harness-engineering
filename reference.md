...
- [Anthropic — Claude Code auto mode: a safer way to skip permissions](https://www.anthropic.com/engineering/claude-code-auto-mode) (Mar 2026) — A third permission management approach beyond sandboxing and `--dangerously-skip-permissions`: learned policies via a transcript-monitoring classifier that auto-approve 93% of safe permission prompts. The classifier watches agent actions in real-time and blocks only risky operations (scope escalation, unknown infrastructure, hostile-content-driven actions), eliminating approval fatigue while preserving safety. Key harness design insight: deterministic rules (hooks, sandboxing) handle known-safe/known-dangerous cases; learned policies handle the ambiguous middle ground.
...
- [Anthropic — Introducing advanced tool use on the Claude Developer Platform](https://www.anthropic.com/engineering/advanced-tool-use) (Nov 2025) — Streaming tool use, parallel tool calls, and programmatic tool execution on the API — infrastructure primitives that enable faster agent loops and more responsive harnesses.
...
- [Anthropic — Measuring AI agent autonomy in practice](https://www.anthropic.com/research/measuring-agent-autonomy) (Feb 2026) — Empirical analysis of agent autonomy on the Claude API: most agent actions are low-risk and reversible, agents are used in risky domains but not yet at scale. Provides a framework for categorizing agent action risk levels and measuring real-world autonomy patterns.
...
- [Anthropic — Building agents with the Claude Agent SDK](https://www.anthropic.com/engineering/building-agents-with-the-claude-agent-sdk) (Sep 2025) — Core agent loop philosophy: gather context → take action → verify work → repeat. Covers agentic search (file system as context engineering), subagents for parallelization and context isolation, automatic compaction for long-running agents, tools as primary action surface, MCP for standardized integrations, and three verification approaches (rules-based, visual feedback, LLM-as-judge). Originally the Claude Code SDK, renamed to Claude Agent SDK to reflect broader applicability beyond coding.
- [Anthropic — Enabling Claude Code to work more autonomously](https://www.anthropic.com/news/enabling-claude-code-to-work-more-autonomously) (Sep 2025) — Product-level introduction of SDK support for subagents and hooks, making agent workflows customizable for specific domains. Complements the Claude Agent SDK engineering article with the product rationale: hooks as deterministic guardrails, subagents as context-isolating delegation, and the extensibility surface that enables teams to encode domain-specific harness patterns.
...
### 21. Escalate, Don't Guess
The most valuable agent skill is knowing when to stop and ask the human. Irreversible operations (data migrations, production deployments, public API changes), security-sensitive changes (auth, secrets, permissions), and ambiguous requirements where two valid interpretations lead to very different implementations — all warrant human confirmation. Autonomous execution on easy-to-verify tasks, human escalation on hard-to-reverse ones. This aligns with Anthropic's Trend 4 (2026 Agentic Coding Trends Report): the most productive human-agent collaboration is when the agent learns "when to ask for help" rather than guessing wrong.

### 22. Build to Delete
Every harness component encodes an assumption about what the model can't do. Design each component to be removable. When a new model lands, test by removing one component at a time — if quality holds, delete it. Anthropic demonstrated this in their March 2026 three-agent harness: after Opus 4.5 shipped, they removed sprint constructs entirely because the model handled longer tasks coherently without forced decomposition. The goal is the simplest harness that achieves the required quality — complexity is a cost, not a feature.
